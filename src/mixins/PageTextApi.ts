...
export default class PageTextApi extends Api<PageText> {
  ...
  async getFullTimePageText(): Promise<Array<PageText>> {
    return (await this.axios.get<Array<PageText>>(`${this.path}/frontpage/vz`)).data;
  }

  @ErrorHandler()
  async getPartTimeFirstElection(): Promise<Array<PageText>> {
    return (await this.axios.get<Array<PageText>>(`${this.path}/frontpage/tz1`)).data;
  }

  @ErrorHandler()
  async getPartTimeSecondElectionText(): Promise<Array<PageText>> {
    return (await this.axios.get<Array<PageText>>(`${this.path}/frontpage/tz2`)).data
  }

  @ErrorHandler()
  async getAnonymousPageText(): Promise<Array<PageText>> {
    // eslint-disable-next-line prettier/prettier
    return (await this.axios.get<Array<PageText>>(`${this.path}/frontpage/anonym`)).data;
  }
}
